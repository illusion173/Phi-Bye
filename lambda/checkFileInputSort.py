...
def move_file_to_error(source_bucket, source_key):
    ...
    fail_bucket = "errorbucketphi-illusjw"
...
def move_file(source_bucket, source_key, destination_bucket, file, file_extension):
    final_bucket = "result-bucket-illusjw"
...
def move_to_worker_text(
    ...
    try:
        response_landing = s3_client.delete_object(
            Bucket="inputbucket-illusjw", Key=source_key
        )
    except Exception as e:
        print(e)
        return e
...
def lambda_handler(event, context):
    ...
    text_input_bucket_name = "in-text-phi"
    audio_input_bucket_name = "in-audio-phi"
